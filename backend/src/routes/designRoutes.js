...
const replicate = new Replicate({
  auth: process.env.REPLICATE_API_TOKEN, // set in your .env
});
...
try {
  ...
  prediction = await replicate.wait(prediction); // wait for result
  generatedImageBase64 = prediction.output.generatedImage;
} catch (err) {
