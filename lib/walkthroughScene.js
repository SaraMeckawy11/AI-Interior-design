...
(function () {
  ...
  post({
    ...
    catalogRequested: Object.keys((payload || {}).catalog || {}).length,
...
(function () {
   ...
   * builds, so furnishing itself can stay synchronous.
   *
   * A model that fails here is not silently dropped: without it the piece falls
   * back to procedural geometry, which looks exactly like the catalogue never
   * having been wired up at all. The counts go back to the app so that
  ...
  window.__LIVINAI_PREPARE__ = function (THREE, GLTFLoader, engine) {
    ...
    if (!GLTFLoader) {
      note('No glTF loader on the "' + engine + '" engine — ' + keys.length + ' catalogue models skipped, using procedural furniture.');
      return start();
    }
    ...
    var settle = function () {
      if (--pending > 0) return;
      if (failures.length) note('Catalogue models failed to parse: ' + failures.join(', '));
      start();
    };
    keys.forEach(function (key) {
      try {
        ...
        loader.parse(bytes.buffer, '', function (gltf) {
          ...
          failures.push(key + ' (' + ((error && error.message) || 'parse error') + ')');
          settle();
        });
      } catch (error) {
