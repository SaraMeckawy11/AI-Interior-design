...
export const WALKTHROUGH_RENDERER_REVISION = "livinai-mobile-walkthrough-v1";
...
(function () {
  ...
  function roomFurniture(type, bounds, style, context) {
    ...
    if (lower.indexOf('living') >= 0) {
      ...
      if (bounds.area > 9) items.push({ model: 'island', name: 'Kitchen island', material: 'Cabinetry and worktop', detail: 'A central preparation surface with clear working space around it.', size: [Math.min(2.0, bounds.width * 0.56), 0.95, 0.9], position: offset(0, 0.05), rotation: rotation, colorKey: 'wood' });
  ...
  function focusRoom(index) {
    ...
    var target = bounds.centre.clone().addScaledVector(bounds.axisZ, -bounds.depth * 0.45);
    var direction = new THREE.Vector3(target.x - camera.position.x, 1.15 - camera.position.y, target.z - camera.position.z);
  ...
  boundsList.forEach(function (bounds, index) {
    ...
    specs.forEach(function (spec) { objectCount += 1; addProceduralModel(apartment, spec, palette); });
  ...
  window.LivinaiScene = {
    ...
    capture: function () {
      ...
      try { post({ type: 'snapshot', image: renderer.domElement.toDataURL('image/jpeg', 0.86) }); }
