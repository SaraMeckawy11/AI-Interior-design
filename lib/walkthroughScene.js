 ...
 *
 *  1. The matching Livinai_web GLB catalogue ships inside the app instead of
 *     being fetched from '/models'. The bytes are identical; only the delivery
 *     differs — see 'catalogKeysFor' and the streaming protocol below.
 ...
 *
 * ## How the catalogue reaches the scene
 *
 * The models used to be base64-inlined into the HTML string. 'chair-modern.glb'
 * is 5.4 MB of base64 on its own, and a document that large is slow to hand to
 * the WebView at best and silently truncated at worst — at which point every
 * piece falls back to procedural geometry and the walkthrough looks like the
 * catalogue was never wired up.
 *
 * So the document now boots small. It draws the architecture and stages every
 * piece procedurally straight away, publishes the catalogue keys it wants, and
 * React Native streams those models in as chunked 'installModel' calls. Each
 * model that arrives replaces its procedural stand-ins in place, keeping any
 * position the user had already nudged it to.
 */

// Bumped whenever the renderer changes in a way that should invalidate a
// cached scene. It used to have to match Livinai_web's own revision string so
// both apps hit the same Interior_Plan export cache; nothing is shared with a
// server any more, so it is only this app's own version now.
export const WALKTHROUGH_RENDERER_REVISION = "livinai-local-catalogue-v1";

/**
 * Which catalogue models each room programme can actually ask for.
 *
 * Each list is the intersection of what 'roomFurniture' emits for that room
 * type and what CATALOG_ALIAS (Livinai_web's MODEL_PATHS) resolves. Streaming a
 * model a plan can never draw is pure cost, so unknown room types fall back to
 * the union rather than every room paying for every model.
 */
const CATALOG_BY_ROOM = {
  "living room": ["sofa", "compactSofa", "armchair", "coffeeTable"],
  "living + dining": [
    "sofa", "compactSofa", "armchair", "coffeeTable",
    "diningTable", "diningChair", "tvUnit",
  ],
  // A salon is a living room that never eats, so it never needs the table.
  salon: ["sofa", "compactSofa", "armchair", "coffeeTable"],
  "salon + dining": [
    "sofa", "compactSofa", "armchair", "coffeeTable",
    "diningTable", "diningChair", "tvUnit",
  ],
  ...
  balcony: ["diningChair", "coffeeTable"],
  laundry: ["bookcase"],
  utility: ["bookcase"],
  closet: ["bookcase"],
  entryway: ["bookcase"],
...
const CATALOG_REACHABLE = [
  "sofa", "compactSofa", "armchair", "coffeeTable", "bed",
  "island", "fridge", "shower", "bathtub", "vanity", "toilet",
  "diningTable", "diningChair", "tvUnit", "workDesk", "modernChair", "bookcase",
];

/**
 * The catalogue keys a plan can draw, cheapest first.
 *
 * Order matters: the models arrive one at a time, so sending the 8 KB kitchen
 * island before the 4 MB desk chair means the room fills in as the user is
 * still looking around rather than all at once at the end.
 */
export function catalogKeysFor(roomConfigs = []) {
  ...
  return [...keys].sort(
    (one, two) => (CATALOG_WEIGHT[one] || 50) - (CATALOG_WEIGHT[two] || 50),
  );
}

/** Rough .glb size in KB, only ever used to order the stream. */
const CATALOG_WEIGHT = {
  island: 6, diningTable: 8, workDesk: 8, coffeeTable: 8, armchair: 9,
  compactSofa: 9, diningChair: 12, vanity: 15, tvUnit: 16, bookcase: 18,
  toilet: 18, bed: 23, fridge: 30, bathtub: 35, shower: 51,
  sofa: 3076, modernChair: 4029,
};
 ...
 * What a room can be.
 *
 * Two of these names carry a decision rather than just a label, and both are
 * here because the old list made people guess:
 *
 * **"Living + Dining"** is the living room that eats. A plain "Living Room"
 * used to silently become one whenever the plan had no dining room drawn — so
 * whether your living room came back with a dining table in the middle of it
 * depended on a rule nobody was told. Now it is a choice with its own name on
 * it: pick this one and the table is yours, pick "Living Room" and it is a
 * lounge. (A plan drawn before this option existed still gets a table in its
 * main living room, so nothing that already works stops working.)
 *
 * **"Salon"** is the formal seating room — the reception room kept for guests,
 * sofas around the walls, floor left open in the middle. It is a living room
 * that is never the room that eats, which is exactly what the second seating
 * room in a home needs to be, and there was previously no way to say it.
 *
 * **"Salon + Dining"** is that same reception room with the formal table in it,
 * which is how a great many homes are actually arranged: guests are received
 * and fed in the same room, and the family's own living room stays a living
 * room. Naming it is the only way to get the table there, because the automatic
 * pick deliberately never chooses a salon while any everyday seating room is
 * available to take the table instead.
 */
...
/**
 * What each newly drawn room is called before anyone names it.
 *
 * Kept separate from ROOM_TYPES because the two lists answer different
 * questions. ROOM_TYPES is ordered so the choices people reach for are near the
 * front; this is ordered like an actual flat, so drawing five rooms gives you a
 * living room, a bedroom, a kitchen and a bathroom rather than three variations
 * on a lounge. Cycling the option list for defaults tied the two together, and
 * adding an option to one silently changed the other.
 */
export const DEFAULT_ROOM_SEQUENCE = [
  "Living Room",
  "Bedroom",
  "Kitchen",
  "Bathroom",
...
/**
 * The styles that actually produce a different home.
 *
 * There were ten, and two of them were duplicates all the way down. The
 * renderer resolves a style to a palette and to a furniture family, and:
 *
 *   Contemporary -> modern palette, modern family   — identical to Modern
 *   Traditional  -> classic palette, classic family — identical to Classic
 *
 * The only thing separating each pair was the random seed, which picks a
 * different silhouette from the same allowed set. That is a coin flip, not a
 * design decision, and offering it as one meant two of the ten choices could
 * not be told from their neighbours — which is exactly what makes a control
 * feel decorative.
 *
 * Every style left resolves to its own combination. Mid-Century shares Modern's
 * palette but has its own furniture family, so it stays.
 *
 * The renderer still understands the removed names, so a plan saved under
 * Contemporary or Traditional reopens and rebuilds exactly as it was.
 */
...
export const DEFAULT_WALKTHROUGH_SETTINGS = {
  // One style for the whole home. It used to be asked per room, which meant a
  // five-room flat presented the same twelve-option list five times and let
  // someone build a Japandi bedroom off an Industrial hallway — a combination
  // nobody chooses on purpose and the app then had to furnish.
  style: "Modern",
  ...
  useCatalog: true,
  /**
   * Walls are solid unless the user says otherwise.
   *
   * This defaulted to true, which meant every walkthrough started with the one
   * thing a walkthrough is for turned off: the camera slid through walls, so
   * aiming at a doorway meant nothing, a corridor had no edges to follow, and
   * the usual result of holding the stick was ending up outside the building
   * looking back at it with no idea which way was in.
   *
   * The exporter has always returned a proper walkable area for this — each room
   * inset by a body radius, joined to the next only through the door openings —
   * and it was going unused. With walls solid the camera has something to slide
   * along, which is what makes a doorway findable.
   */
  freeExplore: false,
  notes: "",
};
 ...
 */
export function buildLayout({ rooms = [], doors = [], windows = [], balconies = [], wallOpenings = [], width, height, pixelsPerMeter }) {
  const toPoint = (point) =>
    Array.isArray(point)
  ...
  return {
    ...
    // Cased openings are cut into walls exactly like doors, and the exporter
    // merges them back into the door list to do it. They travel separately only
    // so it also knows which of those cuts have no door in them — see the
    // circulation guard in the renderer.
    wallOpenings: wallOpenings.map(toSegment),
  };
}
...
(function () {
  ...
  var note = function (message) { post({ type: 'diagnostic', message: message }); };
  ...
  /**
   * Keep every piece inside the room it belongs to.
   *
   * The room programme places furniture against 'bounds', which is the room's
   * oriented bounding box. For a rectangle that is the room; for an L, a bay or
   * anything the plan editor's outline tool can draw, the box is bigger than the
   * floor, and a wardrobe placed at "width/2 - 0.95" ends up buried in a wall or
   * standing in the corridor next door. That is the misplacement you see.
   *
   * Rather than rewrite the programme, each spec is checked against the actual
   * floor polygon after the fact. A piece is clamped into the oriented box with
   * a wall clearance, then, if its footprint still crosses the outline, drawn
   * back toward the room centre until it does not. Pieces the programme already
   * placed correctly — the overwhelming majority — are not moved at all.
   */
  var WALL_CLEARANCE = 0.08; // half a wall thickness plus a little air

  function footprintExtent(spec, bounds) {
    var turn = (spec.rotation || 0) - bounds.rotation;
    var cos = Math.abs(Math.cos(turn));
    var sin = Math.abs(Math.sin(turn));
    var halfWidth = spec.size[0] / 2;
    var halfDepth = spec.size[2] / 2;
    return {
  ...
  function footprintInside(localX, localZ, extent, bounds, polygon) {
    for (var cx = -1; cx <= 1; cx += 2) {
      for (var cz = -1; cz <= 1; cz += 2) {
        var corner = bounds.centre.clone()
          .addScaledVector(bounds.axisX, localX + cx * extent.x)
          .addScaledVector(bounds.axisZ, localZ + cz * extent.z);
        if (!pointInPolygon(corner.x, corner.z, polygon)) return false;
      }
    }
    return true;
  }

  function containInRoom(specs, bounds, polygon) {
    specs.forEach(function (spec) {
      var extent = footprintExtent(spec, bounds);
      var offsetVector = spec.position.clone().sub(bounds.centre);
      var localX = offsetVector.dot(bounds.axisX);
      var localZ = offsetVector.dot(bounds.axisZ);

      // A piece wider than its room cannot be contained; leave it centred
      // rather than clamping it to a nonsensical corner.
      var limitX = Math.max(0, bounds.width / 2 - extent.x - WALL_CLEARANCE);
      var limitZ = Math.max(0, bounds.depth / 2 - extent.z - WALL_CLEARANCE);
      localX = Math.max(-limitX, Math.min(limitX, localX));
      localZ = Math.max(-limitZ, Math.min(limitZ, localZ));

      // Non-convex rooms need the outline itself, not just its bounding box.
      if (!footprintInside(localX, localZ, extent, bounds, polygon)) {
        var pulled = false;
        for (var step = 1; step <= 8; step += 1) {
          var factor = 1 - step / 8;
          if (footprintInside(localX * factor, localZ * factor, extent, bounds, polygon)) {
            localX *= factor;
            localZ *= factor;
            pulled = true;
            break;
        ...
        // Nothing fits anywhere along that line — the room centre is the least
        // wrong answer, and it is at least somewhere the user can see and move.
        if (!pulled) { localX = 0; localZ = 0; }
      }

      spec.position = bounds.centre.clone()
        .addScaledVector(bounds.axisX, localX)
        .addScaledVector(bounds.axisZ, localZ);
    });
    return specs;
  }

  // ── Catalogue furniture ─────────────────────────────────────────────────
  // The same .glb meshes the Livinai web studio serves from /models, streamed
  // in from the app bundle after boot and parsed with the same GLTFLoader.

  // Filled by installModel() as each model arrives, then cloned per piece — the
  // same shape as the web build's loadModel(path) + source.clone(true).
  ...
  /**
   * Which programme item draws which catalogue mesh.
   *
   * Livinai_web declares a 'MODEL_PATHS' entry for sofa, compactSofa, armchair,
   * coffeeTable, diningTable and bed — and then never reaches any of them,
   * because its 'roomFurniture' emits the variant-family names ('sofaVariant',
   * 'bedVariant', …) that MODEL_PATHS has no key for. Those pieces silently
   * resolve to procedural geometry in the web studio's own preview: a living
   * room comes out as a stack of upholstered boxes.
   *
   * So the families are resolved here, using exactly the models Livinai_web
   * ships and exactly the sizes, positions and rotations its 'roomFurniture'
   * computes. Where a variant is a genuinely different object rather than a
   * different finish — a canopy bed, a nesting coffee table, a wall-panelled
   * media unit — it keeps its procedural build, because a single mesh would
   * throw the variation away. Everything else gets the modelled silhouette.
   *
   * 'roundDiningTable' has no entry for the same reason it is null in the web:
   * there is no round table in the package.
   */
  var CATALOG_ALIAS = {
    sofa: 'sofa',
  ...
  /** Variants whose shape is carried by geometry the catalogue has no mesh for. */
  var PROCEDURAL_VARIANTS = {
    sofaVariant: ['left-chaise', 'right-chaise', 'modular'],
    coffeeTableVariant: ['round', 'nesting', 'glass-frame', 'sculptural', 'plinth', 'stone-frame'],
    bedVariant: ['canopy', 'timber-slat', 'wingback', 'sleigh'],
    diningTableVariant: ['round', 'oval']
  };

  function keepsProceduralBuild(spec) {
    var blocked = PROCEDURAL_VARIANTS[spec.model];
    if (!blocked) return false;
    var variant = spec.variant || spec.shape || '';
    return blocked.indexOf(variant) >= 0;
  }

  /**
   * The catalogue key a spec draws, or null when it stays procedural.
   *
   * Sofas are the one size-dependent choice: the package has a full three-seat
   * 'sofa-modern' and a 'sofa-compact', and the room programme already sized the
   * slot, so a narrow room gets the compact model rather than a squashed one.
   */
  function catalogKeyFor(spec) {
    if (spec.catalogKey) return spec.catalogKey;
    if (keepsProceduralBuild(spec)) return null;
    if (spec.model === 'sofaVariant') return spec.size[0] >= 1.9 ? 'sofa' : 'compactSofa';
    if (spec.model === 'armchairVariant') return 'armchair';
    if (spec.model === 'coffeeTableVariant') return 'coffeeTable';
    if (spec.model === 'bedVariant') return 'bed';
    if (spec.model === 'diningTableVariant') return 'diningTable';
    // 'tvUnitVariant' is deliberately absent. It is a composite — cabinet,
    // screen, and for some variants a full media wall — and tv-unit.glb is the
    // cabinet alone, so swapping it in would take the television off the wall.
    // The plain 'tvUnit' (the dining sideboard) is a cabinet, and does map.
    return CATALOG_ALIAS[spec.model] || null;
  }

  /**
   * Pieces staged procedurally that are still waiting for their model, keyed by
   * catalogue key. Furnishing never blocks on the stream: the room is complete
   * and walkable from the first frame, and each model upgrades its pieces as it
   * lands.
   */
  var awaitingModel = {};

  /** Catalogue mesh where one exists and has arrived, procedural otherwise. */
  function addFurniture(parent, spec, palette) {
    var key = settings.useCatalog === false ? null : catalogKeyFor(spec);
    if (key && CATALOG[key]) {
      var placed = addCatalogModel(parent, Object.assign({}, spec, { catalogKey: key }), palette);
      if (placed) { catalogUsed += 1; return placed; }
    }
    var stagedPiece = addProceduralModel(parent, spec, palette);
    if (key && expectedModels.indexOf(key) >= 0) {
      (awaitingModel[key] = awaitingModel[key] || []).push({
        parent: parent, spec: spec, palette: palette, staged: stagedPiece
      });
    }
    return stagedPiece;
  }
  ...
  /**
   * Swap every stand-in for 'key' with the real mesh.
   *
   * The transform is carried across rather than recomputed: by the time a large
   * model arrives the user may already have nudged or rotated the stand-in, and
   * having a piece jump back to its default the moment its model loads would be
   * worse than never loading it.
   */
  function upgradeStagedFurniture(key) {
    var waiting = awaitingModel[key];
    if (!waiting || !waiting.length) return 0;
    delete awaitingModel[key];
    var upgraded = 0;
    waiting.forEach(function (entry) {
      var staged = entry.staged;
      var replacement = addCatalogModel(
        entry.parent, Object.assign({}, entry.spec, { catalogKey: key }), entry.palette
      );
      if (!replacement) return;
      replacement.position.copy(staged.position);
      replacement.rotation.y = staged.rotation.y;
      replacement.userData.home = staged.userData.home;
      replacement.userData.editId = staged.userData.editId;
      replacement.traverse(function (child) {
        if (child.userData) {
          child.userData.home = staged.userData.home;
          child.userData.editId = staged.userData.editId;
        }
      });
      if (engine.selected === staged) {
        engine.selected = replacement;
        if (engine.selectionBox) scene.remove(engine.selectionBox);
        engine.selectionBox = new THREE.BoxHelper(replacement, 0xb0653f);
        scene.add(engine.selectionBox);
      }
      furnitureIndex.forEach(function (item) { if (item.group === staged) item.group = replacement; });
      entry.parent.remove(staged);
      disposeGroup(staged);
      catalogUsed += 1;
      upgraded += 1;
    });
  ...
  function disposeGroup(group) {
    group.traverse(function (child) {
      ...
      (Array.isArray(child.material) ? child.material : [child.material])
        .forEach(function (material) { material.dispose(); });
    });
  }
  ...
  function addProceduralModel(parent, spec, palette) {
    ...
    if (spec.model === 'sofaVariant') {
      ...
      // The media wall goes behind the screen. Livinai_web puts it at
      // -depth/2 - 0.03, which is 5 cm nearer the room than the television —
      // so the 'timber wall' and 'stone wall' living rooms render as a blank
      // panel with the TV hidden inside it.
      if (/timber-wall|stone-wall/.test(mediaVariant)) addBox([width * 0.96, height * 1.34, 0.045], [0, height * 0.83, -depth / 2 + 0.06], mediaVariant === 'stone-wall' ? stoneMaterial : darkWoodMaterial);
  ...
  // ── What a room name means ──────────────────────────────────────────────
  // The same vocabulary the renderer uses (plan_walkthrough.is_lounge_room and
  // friends), so the preview and the finished scene agree about which room is
  // a lounge, which is a balcony, and which one eats.
  function isBalconyType(type) { return /balcony|terrace|veranda/i.test(type || ''); }
  function isLoungeType(type) { return /living|lounge|salon|sitting|reception|studio/i.test(type || ''); }
  function isFormalLoungeType(type) { return /salon|reception/i.test(type || ''); }
  function isDedicatedDiningType(type) { return /dining/i.test(type || '') && !isLoungeType(type); }
  function isExplicitDiningLounge(type) { return /dining/i.test(type || '') && isLoungeType(type); }
  ...
  function roomFurniture(type, bounds, style, context) {
    ...
    if (isBalconyType(lower)) {
      // Outdoor floor: somewhere to sit, something to put a cup on, planting.
      // No sofa and no media wall — the thing that makes a balcony read as a
      // small indoor room is furnishing it like one.
      var balconyDeep = Math.min(bounds.width, bounds.depth) >= 1.45;
      if (balconyDeep) {
        items.push({ model: 'coffeeTableVariant', variant: 'round', name: 'Bistro table', material: 'Powder-coated metal and stone', detail: 'A small outdoor table squared to the balcony’s long axis.', size: [0.56, 0.72, 0.56], position: offset(0, 0), rotation: rotation, colorKey: 'wood' });
        [-0.62, 0.62].forEach(function (x) {
          items.push({ model: 'diningChair', name: 'Balcony chair', material: 'Weatherproof textile and metal', detail: 'Paired across the bistro table with the doorway left clear.', size: [0.46, 0.86, 0.48], position: offset(x, 0), rotation: rotation + (x > 0 ? -Math.PI / 2 : Math.PI / 2), colorKey: 'fabric' });
        });
      } else {
        items.push({ model: 'coffeeTableVariant', variant: 'plinth', name: 'Balcony bench', material: 'Weathered timber', detail: 'A narrow balcony seats along its length, not across it.', size: [Math.min(1.35, bounds.width * 0.7), 0.44, 0.38], position: offset(0, -bounds.depth / 2 + 0.34), rotation: rotation, colorKey: 'wood' });
      }
    } else if (isLoungeType(lower)) {
  ...
  // Catalogue keys React Native is going to stream in. A piece whose key is not
  // on this list is never staged for an upgrade, so it will not sit waiting for
  // a model that is never coming.
  var expectedModels = payload.catalogKeys || [];
  ...
  var objectCount = 0;
  var furnitureIndex = [];
  var hasDedicatedDining = roomConfigs.some(function (room) { return isDedicatedDiningType((room && room.roomType) || ''); });
  var kitchenBounds = boundsList.filter(function (_, index) { return /kitchen/i.test((roomConfigs[index] && roomConfigs[index].roomType) || ''); });

  // Which single room eats, decided once with every room in view — the same
  // rule the renderer applies. A room the user named "Living + Dining" wins
  // outright; failing that the largest everyday seating room big enough to
  // hold a table as well as a sofa; and if a dining room was drawn, none of
  // them. A second living room, a salon, and a small room are never it.
  //
  // The two floors are the drawn area of the room. They are the renderer's
  // LIVING_DINING_MIN_AREA and EXPLICIT_LIVING_DINING_MIN_AREA expressed in
  // this space rather than in its boosted one — the renderer measures a shell
  // about 1.66x the area of the plan, this preview measures the plan itself.
  var DINING_HOST_MIN_AREA = 13;
  var EXPLICIT_DINING_MIN_AREA = 9.5;
  var diningHosts = {};
  var explicitHosts = boundsList
    .map(function (bounds, index) { return { index: index, area: bounds.area }; })
    .filter(function (entry) {
      return isExplicitDiningLounge((roomConfigs[entry.index] || {}).roomType || '') && entry.area >= EXPLICIT_DINING_MIN_AREA;
    });
  if (explicitHosts.length) {
    explicitHosts.forEach(function (entry) { diningHosts[entry.index] = true; });
  } else if (!hasDedicatedDining) {
    var candidates = boundsList
      .map(function (bounds, index) { return { index: index, area: bounds.area }; })
      .filter(function (entry) {
        var name = (roomConfigs[entry.index] || {}).roomType || '';
        return isLoungeType(name) && !isBalconyType(name) && entry.area >= DINING_HOST_MIN_AREA;
      });
    var everyday = candidates.filter(function (entry) {
      return !isFormalLoungeType((roomConfigs[entry.index] || {}).roomType || '');
    });
    var pool = everyday.length ? everyday : candidates;
    if (pool.length) {
      diningHosts[pool.reduce(function (best, entry) {
        return entry.area > best.area ? entry : best;
      }).index] = true;
    }
  }
  ...
  boundsList.forEach(function (bounds, index) {
    var config = roomConfigs[index] || { style: 'Modern', roomType: 'Living Room' };
    var palette = paletteFor(config.style, settings.colorMood);
    var isLiving = isLoungeType(config.roomType || '') && !isBalconyType(config.roomType || '');
    var hostsDining = !!diningHosts[index];
    var hasBalcony = balconySegments.some(function (segment) {
      var midpoint = segment[0].clone().add(segment[1]).multiplyScalar(0.5);
      return midpoint.x >= bounds.minX - 0.35 && midpoint.x <= bounds.maxX + 0.35 && midpoint.z >= bounds.minZ - 0.35 && midpoint.z <= bounds.maxZ + 0.35;
    });
    var loungeOffsetX = 0;
    if (hostsDining && kitchenBounds.length) {
    ...
    var specs = roomFurniture(config.roomType, bounds, config.style || 'Modern', { hasBalcony: hasBalcony, loungeOffsetX: loungeOffsetX, openPlan: hostsDining });
    if (settings.designProfile === 'Airy') specs = specs.slice(0, Math.max(1, Math.ceil(specs.length * 0.62)));
    specs = containInRoom(specs, bounds, polygons[index]);
    ...
    if (!/minimal/i.test(settings.decorSet || '') && !/bath|utility|laundry|closet/i.test(config.roomType || '')) {
      ...
      // A corner of the room, not a corner of its world-space bounding box —
      // for a rotated or L-shaped room those are different places, and the
      // second one is often outside the floor.
      var plantSpec = {
        size: [0.5, 1.3, 0.5],
        rotation: bounds.rotation,
        position: bounds.centre.clone()
          .addScaledVector(bounds.axisX, -bounds.width / 2 + 0.45)
          .addScaledVector(bounds.axisZ, -bounds.depth / 2 + 0.45)
      };
      containInRoom([plantSpec], bounds, polygons[index]);
      plant.position.copy(plantSpec.position);
  ...
  // Report what the furniture pipeline actually managed to do. A catalogue that
  // silently fails to load looks identical to one that was never wired up — the
  // procedural families just take over — so the counts are surfaced rather than
  // guessed at.
  // Models that have finished streaming, whether or not they parsed. A model
  // that fails is still finished: counting only successes would leave the app
  // reporting "still loading" for as long as the walkthrough is open.
  var catalogSettled = 0;

  function reportSceneState(type) {
    post({
      type: type,
      objects: objectCount,
      rooms: layout.rooms.length,
      engine: window.__LIVINAI_ENGINE__ || 'unknown',
      threeVersion: THREE.REVISION,
      catalogRequested: expectedModels.length,
      catalogSettled: catalogSettled,
      catalogLoaded: Object.keys(CATALOG).length,
      catalogUsed: catalogUsed
    });
  }

  /**
   * Receive one streamed .glb and upgrade every piece waiting on it.
   *
   * React Native calls this with base64 chunks because a multi-megabyte string
   * is not something to hand across the bridge in one piece. The final chunk
   * carries 'last', at which point the model is assembled, parsed and applied.
   */
  var incoming = {};
  window.LivinaiScene.installModel = function (key, chunk, last) {
    incoming[key] = (incoming[key] || '') + chunk;
    if (!last) return;
    var base64 = incoming[key];
    delete incoming[key];
    var settle = function () {
      catalogSettled += 1;
      reportSceneState('sceneUpdate');
    };
    if (!window.__LIVINAI_GLTF_LOADER__) {
      note('No glTF loader on the "' + (window.__LIVINAI_ENGINE__ || 'unknown')
        + '" engine — "' + key + '" is showing as procedural furniture.');
      return settle();
    }
    try {
      ...
      new window.__LIVINAI_GLTF_LOADER__().parse(bytes.buffer, '', function (gltf) {
        CATALOG[key] = gltf.scene;
        upgradeStagedFurniture(key);
        settle();
      }, function (error) {
        note('Furniture model "' + key + '" could not be read: ' + ((error && error.message) || 'parse error'));
        settle();
      });
    } catch (error) {
      note('Furniture model "' + key + '" could not be decoded: ' + error.message);
      settle();
    }
  };

  reportSceneState('ready');
})();
`;
...
export function buildWalkthroughHtml({
  ...
  const payload = JSON.stringify({
    ...
    roomIndex,
    night,
    catalogKeys: catalogKeysFor(roomConfigs),
  })
    // `</script>` inside a string literal would close the tag early.
    .replace(/</g, "\\u003c");

  const loader = THREE_SOURCES.map((source) => `'${source}'`).join(",");
  // The import map has to be in the document before any module script is
  // fetched, so it is written statically into <head> rather than injected at
  // runtime — injecting it was the fragile part of this.
  const importMap = JSON.stringify({
    imports: {
      three: `${THREE_MODULE_SOURCES[0]}/build/three.module.js`,
      "three/addons/": `${THREE_MODULE_SOURCES[0]}/examples/jsm/`,
    },
  });
...
(function () {
  ...
  function note(message) {
    try { window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'diagnostic', message: message })); } catch (error) {}
  }
  ...
  /**
   * Hand the engine to the scene and start it immediately.
   *
   * Nothing waits on furniture here. The architecture and a full set of
   * procedural stand-ins are drawn on the first frame, and the catalogue models
   * stream in afterwards through 'LivinaiScene.installModel', each replacing its
   * stand-ins as it arrives. A walkthrough that is walkable in a second and
   * sharpens as you look around beats one that stares at a spinner while four
   * megabytes of desk chair decode.
   */
  window.__LIVINAI_PREPARE__ = function (THREE, GLTFLoader, engine) {
    if (window.THREE && started) return;
    window.THREE = THREE;
    window.__LIVINAI_ENGINE__ = engine;
    window.__LIVINAI_GLTF_LOADER__ = GLTFLoader || null;
    window.__LIVINAI_MODELS__ = {};
    if (!GLTFLoader) {
      note('No glTF loader on the "' + engine + '" engine — the walkthrough is using procedural furniture.');
    }
    start();
  };

  /**
   * Fallback engine: UMD three, which has no GLTFLoader — three dropped its
   * non-module example builds after r147. The scene still renders; its
   * catalogue pieces become procedural, and the diagnostic above says so.
   */
  function umd() {
    ...
    script.onload = function () { window.__LIVINAI_PREPARE__(window.THREE, null, 'umd'); };
    script.onerror = umd;
    document.head.appendChild(script);
  }

  // The module bootstrap below is deferred, so give it a chance to run before
  // deciding the WebView cannot handle it. A module whose imports fail to
  // resolve never executes and does not reliably fire onerror.
  window.__LIVINAI_MODULE_FAILED__ = umd;
  setTimeout(function () { if (!started && !window.THREE) { note('The module engine did not start; falling back to UMD three without a glTF loader.'); umd(); } }, 15000);
})();
</script>
<script type="module">
  import * as THREE from 'three';
  import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
  window.__LIVINAI_PREPARE__(THREE, GLTFLoader, 'module');
</script>
<script>
  window.addEventListener('error', function (event) {
    if (event && event.target && event.target.tagName === 'SCRIPT') window.__LIVINAI_MODULE_FAILED__();
  }, true);
</script>
<script>
