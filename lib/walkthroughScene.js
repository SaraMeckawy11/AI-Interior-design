...
(function () {
   ...
   */
  var CATALOG_ALIAS = {
    // Variant families intentionally stay procedural. This is how
    // Livinai_web renders its original style-aware sofa, chair, bed, coffee
    // table and dining families; routing them to one generic GLB erased the
    // selected silhouette and often made the object fit much smaller than its
    // reserved real-world dimensions.
  ...
  function roomFurniture(type, bounds, style, context) {
    ...
    if (lower.indexOf('living') >= 0) {
      ...
      items.push({ model: 'bedVariant', variant: bedVariant, name: bedVariant.replace('-', ' ') + ' bed', material: style + ' upholstery and timber', detail: 'Centred on the longest wall with balanced bedside clearance.', size: [bedWidth, 1.0, bedDepth], position: offset(0, -bounds.depth / 2 + bedDepth / 2 + 0.12), rotation: rotation, colorKey: 'fabric' });
    ...
    polygon.forEach(function (start, edgeIndex) {
      ...
      openings.forEach(function (opening) {
        ...
        if (opening.type === 'door') {
          var doorHeight = Math.max(1.95, Math.min(2.45, Number(opening.settings.height) || 2.1));
  ...
  boundsList.forEach(function (bounds, index) {
    ...
    specs.forEach(function (spec, specIndex) {
      ...
      var editId = index + ':' + specIndex + ':' + spec.model;
