...
export const LIVINAI_WEB_RENDERER_REVISION = "door-corridors-no-throw-pillows-v34";
...
(function(){
  ...
  var state={mode:payload.mode||'walk',night:!!payload.night,roomIndex:payload.roomIndex||0,freeExplore:settings.freeExplore!==false,hiddenWalls:[],designer:false};
  ...
  // Walking through walls is what removing one is *for*: the point of taking a
  // wall out is to be able to stand behind where it was.
  var validMove=function(target){return state.freeExplore||state.hiddenWalls.length>0||(data.walkable||[]).some(function(poly){return pointInPolygon(target.x,target.z,poly);});};
  ...
  /**
   * The walls around the room being looked at, named from where the camera is.
   *
   ...
   *
   * Removing walls automatically turned out to be the wrong shape for this: it
   * chose for you, and it chose again every time you turned. So the walls are
   * listed instead, and you pick. They are named the way you would point at
   * them from where you are standing — the wall in front, the one on your left —
   * because that is the only description that needs no map to follow. The list
   * is taken once, when the render sheet opens, so a name cannot change while
   * you are reading it.
   */
  var roomReach=function(index){
    var polygon=(data.walkable||[])[index];var centre=roomCentre(index);
    if(!polygon||!polygon.length)return 4.5;
    var far=0;polygon.forEach(function(point){far=Math.max(far,Math.hypot(point[0]-centre.x,point[1]-centre.z));});
    return far+1.2;
  };
  var wallCentre=new THREE.Vector3();
  var listWalls=function(){
    var centre=roomCentre(state.roomIndex),reach=roomReach(state.roomIndex);
    var forwardX=-Math.sin(engine.yaw),forwardZ=-Math.cos(engine.yaw);
    var rightX=Math.cos(engine.yaw),rightZ=-Math.sin(engine.yaw);
    var bySide={};
    engine.walls.forEach(function(wall,index){
      wall.box.getCenter(wallCentre);
      if(Math.hypot(wallCentre.x-centre.x,wallCentre.z-centre.z)>reach)return;
      var dx=wallCentre.x-camera.position.x,dz=wallCentre.z-camera.position.z;
      var ahead=dx*forwardX+dz*forwardZ,across=dx*rightX+dz*rightZ;
      var side=Math.abs(ahead)>=Math.abs(across)
        ?(ahead>=0?'front':'behind')
        :(across>=0?'right':'left');
      var distance=Math.hypot(dx,dz);
      // One wall per side: the nearest. A room's side is made of several
      // exported pieces once it has a door or a window in it, and offering
      // "Left wall" four times is not a choice.
      if(!bySide[side]||distance<bySide[side].distance)bySide[side]={id:index,distance:distance};
    });
    var walls=Object.keys(bySide).map(function(side){return{id:bySide[side].id,side:side};});
    post({type:'walls',walls:walls});
    return walls;
  };
  var applyWallVisibility=function(){
    if(!engine.walls.length)return;
    engine.walls.forEach(function(wall,index){wall.node.visible=state.hiddenWalls.indexOf(index)<0;});
  };
  ...
  window.LivinaiScene={
    ...
    listWalls:listWalls,
    setHiddenWalls:function(ids){state.hiddenWalls=Array.isArray(ids)?ids:[];applyWallVisibility();},
