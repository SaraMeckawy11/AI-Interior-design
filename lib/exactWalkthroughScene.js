/**
 * Mobile host for Livinai_web's exact Interior_Plan realtime export.
 *
...
</style></head><body><div id="stage"></div><div id="loading">Loading the exact Interior_Plan furniture…</div>
<script>window.__EXACT_LIVINAI__=${payload};</script>
<script src="https://unpkg.com/three@0.147.0/build/three.min.js"></script>
<script>if(!window.THREE){document.write('<script src="https://cdn.jsdelivr.net/npm/three@0.147.0/build/three.min.js"><\\/script>');}</script>
<script>if(!window.THREE){document.write('<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r147/three.min.js"><\\/script>');}</script>
<script src="https://unpkg.com/three@0.147.0/examples/js/loaders/GLTFLoader.js"></script>
<script>if(window.THREE&&!THREE.GLTFLoader){document.write('<script src="https://cdn.jsdelivr.net/npm/three@0.147.0/examples/js/loaders/GLTFLoader.js"><\\/script>');}</script>
<script>
(function(){
  'use strict';
  ...
  var state={mode:payload.mode||'walk',night:!!payload.night,roomIndex:payload.roomIndex||0,freeExplore:settings.freeExplore!==false};
  var engine={yaw:Number(data.spawnYaw)||0,pitch:-0.04,orbitAngle:0.72,orbitPitch:0.72,selected:null,selectionBox:null,drag:false,moved:0,lastX:0,lastY:0,joystick:{x:0,y:0},furniture:[],ceilings:[],overheads:[],clock:new THREE.Clock()};
  ...
  var sun=new THREE.DirectionalLight(0xfff2da,state.night?0.7:1.7);sun.position.set(-5.5,10,7.5);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-16;sun.shadow.camera.right=16;sun.shadow.camera.top=16;sun.shadow.camera.bottom=-16;threeScene.add(sun);
  ...
  var selectGroup=function(group){clearSelection();engine.selected=group;engine.selectionBox=new THREE.BoxHelper(group,0xed6259);threeScene.add(engine.selectionBox);var item=group.userData.item||{};post({type:'select',info:{name:item.label||'Interior_Plan furniture',material:'Interior_Plan native PBR object',detail:'Original Livinai_web furniture, dimensions and placement.',category:'exact-furniture'}});};
  var applyLighting=function(){threeScene.background.set(state.night?0x132333:0xc9d5d8);renderer.toneMappingExposure=state.night?0.84:0.8;hemisphere.intensity=state.night?0.48:0.85;sun.intensity=state.night?0.7:1.7;};
  ...
  var validMove=function(target){return state.freeExplore||(data.walkable||[]).some(function(poly){return pointInPolygon(target.x,target.z,poly);});};
  ...
  window.LivinaiScene={
    ...
    capture:function(purpose){var info=composition();post({type:'composition',composition:info});post({type:'snapshot',purpose:purpose||'photo',composition:info,image:renderer.domElement.toDataURL('image/jpeg',0.92)});}
  };
  ...
  new THREE.GLTFLoader().load(data.modelUrl,function(gltf){
    ...
    document.getElementById('loading').className='done';focusRoom(state.roomIndex);post({type:'ready',objects:engine.furniture.length,rooms:(data.roomCenters||[]).length,exact:true,source:'Interior_Plan'});
  },undefined,function(error){post({type:'error',message:'The exact Interior_Plan scene could not be loaded: '+(error&&error.message?error.message:'model error')});});
  var render=function(){requestAnimationFrame(render);var delta=Math.min(engine.clock.getDelta(),0.05);engine.ceilings.forEach(function(node){node.visible=state.mode!=='plan';});engine.overheads.forEach(function(node){node.visible=state.mode!=='plan';});if(state.mode==='walk'){if(engine.joystick.x||engine.joystick.y){var forward=new THREE.Vector3(-Math.sin(engine.yaw),0,-Math.cos(engine.yaw)),right=new THREE.Vector3(Math.cos(engine.yaw),0,-Math.sin(engine.yaw)),next=camera.position.clone().addScaledVector(forward,-engine.joystick.y*2.35*delta).addScaledVector(right,engine.joystick.x*2.35*delta);if(validMove(next))camera.position.copy(next);}camera.up.set(0,1,0);camera.rotation.set(engine.pitch,engine.yaw,0);}else{var target=state.mode==='plan'?new THREE.Vector3(bounds.cx,0,bounds.cz):roomCentre(state.roomIndex).setY(0.9),distance=state.mode==='plan'?planDistance:Math.max(3.4,Math.max(planWidth,planDepth)*0.55),pitch=state.mode==='plan'?Math.max(0.12,engine.orbitPitch*0.34):engine.orbitPitch,horizontal=distance*Math.sin(pitch);camera.position.set(target.x+horizontal*Math.sin(engine.orbitAngle),target.y+distance*Math.cos(pitch),target.z+horizontal*Math.cos(engine.orbitAngle));camera.up.set(0,1,0);camera.lookAt(target);}engine.selectionBox&&engine.selectionBox.update();renderer.render(threeScene,camera);};render();
}());
</script></body></html>`;
}
