...
export const LIVINAI_WEB_RENDERER_REVISION = "current-furniture-fixed-walls-v38";
...
(function(){
  ...
  var engine={
    ...
    joystick:{x:0,y:0},furniture:[],ceilings:[],overheads:[],userPose:null,clock:new THREE.Clock()};
   ...
   *
   * **Direct manipulation, in both views.** A drag moves the *home*, not the
   * camera: swipe right and the room slides right under your finger. Walking
   * used to do the opposite — swipe right and the camera turned right, so the
   * room shot off to the left — while the bird view already did it this way
   * round. The two halves of one screen disagreed about which way a drag
   * turns, which is why neither felt right.
   *
  ...
  var focusRoom=function(index){state.roomIndex=Math.max(0,Math.min((data.roomCenters||[]).length-1,Number(index)||0));var centre=roomCentre(state.roomIndex);camera.position.set(centre.x,1.62,centre.z);setLook(spawnYaw,-0.04);};
  ...
  var applyPose=function(pose){camera.position.set(pose.x,1.62,pose.z);setLook(pose.yaw,pose.pitch);};
  ...
  window.LivinaiScene={
    ...
    setMode:function(value){state.mode=value;clearSelection();if(value==='walk'){focusRoom(state.roomIndex);}else{engine.panX=0;engine.panZ=0;engine.targetOrbitDistance=planDistance;engine.orbitDistance=planDistance;engine.targetOrbitTilt=0.78;engine.orbitTilt=0.78;}applyLighting();},
  ...
  var stepBy=function(vector,metres){
    if(!metres)return;
    var next=camera.position.clone().addScaledVector(vector,metres);
    if(validMove(next))camera.position.copy(next);
  };
  ...
  canvas.addEventListener('pointermove',function(event){
    ...
    if(list.length===1){
      // One finger: look around. The scene follows the finger in both views.
      if(state.mode==='walk'){
  ...
  var endPointer=function(event){
    ...
    if(!hits.length){clearSelection();post({type:'select',info:null});return;}
    var node=hits[0].object;
    while(node&&!node.userData.item)node=node.parent;
    if(node)selectGroup(node);
  };
    ...
    if(state.mode==='walk'){
      if(engine.joystick.x||engine.joystick.y){
        var forward=new THREE.Vector3(-Math.sin(engine.yaw),0,-Math.cos(engine.yaw)),
            ...
            next=camera.position.clone()
              .addScaledVector(forward,-engine.joystick.y*2.35*delta)
              .addScaledVector(right,engine.joystick.x*2.35*delta);
        if(validMove(next))camera.position.copy(next);
      }
      camera.up.set(0,1,0);
      camera.rotation.set(engine.pitch,engine.yaw,0);
