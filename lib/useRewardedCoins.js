...
export default function useRewardedCoins(token, { enabled = true } = {}) {
  ...
  }, [advertisingReady, claim, enabled]);
  ...
  const watchAd = useCallback(() => {
    ...
    if (!enabled) return;
    if (!advertisingReady) {
      setMessage("Ads are unavailable right now. Please try again in a moment.");
      return;
    }
  ...
  }, [advertisingReady, enabled, status]);
