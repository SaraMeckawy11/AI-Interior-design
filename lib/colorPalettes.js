...
/**
 * The palette flattened to the shape the API takes: three names and three hexes,
 * which is all the prompt engine needs to write the ratio clause.
 */
export function paletteForRequest(tone, palette) {
  const resolved = palette || paletteForTone(tone);
  if (!resolved) return undefined;
  return {
    dominant: resolved.dominant.name,
    secondary: resolved.secondary.name,
    accent: resolved.accent.name,
    dominantHex: resolved.dominant.hex,
    secondaryHex: resolved.secondary.hex,
    accentHex: resolved.accent.hex,
  };
}
