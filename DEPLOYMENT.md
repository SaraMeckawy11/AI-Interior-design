...
|---|---|---|
| Gen‑Klein + ControlNet inference (primary) | Modal | **Yes** |
| Gen‑Klein + ControlNet inference (fallback) | RunPod (`interiorAI`) | **Yes** |
...
## 4. RunPod — the fallback host

`POST /api/designs` calls **Modal first and falls back to RunPod**. Modal serves
every design it can; the `interiorAI/` handler on endpoint `9x2kmfa8z6483c` only
answers when Modal errors or returns no image.

**Both hosts run the same two engines**, routed by the same rule: FLUX.2 [klein]
for photo redesigns, SD 1.5 + depth/seg ControlNets for guided floor plans.
`interiorAI/inference_core.py` is a port of the two engine classes in
...
That is what makes the order a free choice, and it has been made twice. RunPod
was SD 1.5 for *every* request, so putting it first meant nearly every design
came back from an engine the prompts were not written for — the reason it was
moved behind Modal the first time. The port removed that objection; Modal leads
now because it is the host actually provisioned for this work. It answers a warm
request in seconds from volume-cached weights and owns its own capacity, where
RunPod needs a 48 GB GPU class and 16 GB of weights in place (§4a) before it can
answer at all.

Every generation logs the host, the engine and the model:

```
Design generated by modal: engine=gen-klein model=black-forest-labs/FLUX.2-klein-4B imageLen=…
Modal request failed, falling back to RunPod: …
```

### 4a. What the endpoint needs

Two things the SD 1.5-only worker did not. They still apply — a fallback that
cannot start is not a fallback:
...
inference.** The endpoint scales from zero, so a design arriving cold waits for a
worker before anything runs: a measured job spent 59.8s queued and 12.9s
generating. If you change the endpoint's scaling, re-check `RUNPOD_MAX_POLLS`
against it.

That budget is now spent *after* Modal has already failed, so the user has been
waiting a while by the time it starts — but shortening it would mean giving up on
the last host that could still answer. A cold worker also loads FLUX.2 [klein]
rather than SD 1.5 now, and if the weights are on a network volume rather than
baked into the image, the very first worker per volume downloads them too.
...
`RUNPOD_API_KEY` unset the route skips RunPod without making a request, so a
Modal failure returns 502 straight away instead of spending the poll budget on a
host that cannot answer. That is the quickest way to take an unprovisioned or
misbehaving endpoint out of the path.
