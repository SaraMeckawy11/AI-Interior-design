...
The two do not produce the same picture: RunPod is SD 1.5 + ControlNet, Modal is
FLUX.2 [klein]. They share `prompt_engine.py` — the files are byte-identical —
so a design does not change *meaning* when it crosses over, but it does change
