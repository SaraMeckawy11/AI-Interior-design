...
|---|---|---|
| Gen‑Klein (FLUX.2 [klein]) inference (primary) | Modal | **Yes** |
| SD 1.5 + ControlNet inference (fallback) | RunPod (`interiorAI`) | Only if you still use them |
...
## 4. RunPod handlers — the fallback engine
...
RunPod is second despite being the cheaper of the two, because the two do not
produce the same picture: Modal is FLUX.2 [klein], RunPod is SD 1.5 +
ControlNet. They share `prompt_engine.py` — the files are byte-identical — so a
design does not change *meaning* when it crosses over, but it does change
engine. Every generation logs which one answered:

```
Design generated by modal, imageLen=…
Modal request failed, falling back to RunPod: …
```

Putting FLUX.2 [klein] on RunPod as well would remove that difference, but it
needs a 48 GB GPU class enabled on the endpoint — `GenKlein` runs on an L40S in
bf16 with no CPU offload — and a network volume for the 16 GB of weights, since
baking them into the image the way `interiorAI/Dockerfile` bakes SD 1.5 would
make cold starts far worse.
...
`RUNPOD_API_KEY` unset the route skips the fallback entirely and a Modal failure
goes straight to the user, which is how it behaved before the fallback existed.
